...
class Crawler:
    ...
    def __init__(self, is_desktop):
        ...
        self.chrome = webdriver.Chrome(config.get('Setting','CHROME_DRIVER_DIRECTORY'))
    ...
    def __get_new_app_list(self, popular_url):
        ...
        div_app_list =  self.chrome.find_elements_by_css_selector(".card.no-rationale.square-cover.apps.small")
    ...
    def __get_app_detail(self, package_list):
        ...
        for package in package_list:
            ...
            try:
                name = self.chrome.find_element_by_css_selector('.id-app-title').text
                img_src = self.chrome.find_element_by_css_selector('.cover-image').get_attribute('src')
                updated_date = self.chrome.find_elements_by_css_selector('.content')[0].text
            except:
                print(package + " 오류 발생")
